...
app.post('/', function (req, res, next) {
  ...
  function countNumber(assistant) {
    ...
    request('http://ec2-34-210-46-51.us-west-2.compute.amazonaws.com:5001/api/v1/workingNotes', function (error, response) {
      if (error) {
        next(error);
      } else {
        ...
        targetedDocumentReqs = targetedDocuments.map(function (doc) {
          return (Rx.Observable.defer(
            function () {
              return rp({
                method: 'POST',
                uri: 'http://ec2-34-210-46-51.us-west-2.compute.amazonaws.com:5000/api/v1/bff/xeroDraftDocument',
